import com.demo.langserver.DemoLanguageServer;
...
/**
 * Launcher for Demo Language server.
 */
public class StdioLauncher {
    ...
    private static void startServer(InputStream in, OutputStream out) throws ExecutionException, InterruptedException {
        // Initialize the DemoLanguageServer
        DemoLanguageServer demoLanguageServer = new DemoLanguageServer();
        // Create JSON RPC launcher for DemoLanguageServer instance.
        Launcher<LanguageClient> launcher = LSPLauncher.createServerLauncher(demoLanguageServer, in, out);

        // Get the client that request to launch the LS.
        LanguageClient client = launcher.getRemoteProxy();

        // Set the client to language server
        demoLanguageServer.connect(client);
